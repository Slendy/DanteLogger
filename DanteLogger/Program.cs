...
if (args.Length > 0)
{
    ...
    {
        Log.Information("DanteLogger {Tag}, Commit: {GitCommit}", VersionUtil.GetVersion(), ThisAssembly.Git.Commit);
    ...
    if (args.Length == 2 && args[0].Equals("substatus", StringComparison.InvariantCultureIgnoreCase))
    {
        ...
        Log.Information("Highest Channel Number {HighestChannel}", subscriptionStatus.DefaultIfEmpty().Max(s => s?.ChannelNumber));
        ...
        Log.Information("RxChannel backup method count: {RxChCount}", rxChannels.Count);
        return;
    }
